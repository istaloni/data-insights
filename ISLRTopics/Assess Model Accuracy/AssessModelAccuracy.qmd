...
The Bayes classifier assigns each observation to the most likely class given its predictor values, by assigning a test observation with predictor vector $x_0$ to the class $j$ for which 
$$
Pr(Y = j \mid X = x_0)
$$
is largest. This *Bayes classifier* equation describes the probability that $Y = j$, given the observed predictor vector $x_0$. Put simply, if $Pr(Y = j \mid X = x_0)$ > 0.5, then the equation predicts class one, and class two otherwise. For example, see figure: 
...
$$
KNN classifies the test observation $x_0$ to the class with the largest probability. As an example, see figure below: 
![](knn.jpg)
...
We can increase $K$ to produce a method that closely approaches the Bayes classifier. See KNN decision boundary of a larger data set with $K$ = 10: 

![](knn1.jpg) 

* Black curve: KNN decision boundary at $K$ = 10 
* Purple dashed line: Bayes decision boundary 

At the extremes where $K$ is low and high, see below: 
![](knn2.jpg) 

* Left figure: 
  + $K$ = 1, KNN decision boundary is overly flexible, finds patterns in data that do not correspond to Bayes decision boundary 
  + Low bias, High variance 
* Right figure: 
  + $K$ = 100, KNN decision boundary is less flexible, produces a decision boundary that is close to linear 
  + High bias, Low variance 
  
As we decrease $K$, the method becomes more flexible, and training error rate decreases. However, like in the regression setting, the test error rate increases at the end if we use a very flexible model, signaling overfitting. This is elaborated in this figure: 
![](knn3.jpg)
  
::: callout-important
As we use more flexible classification methods, training error rate will decline but the test error rate may not. 
:::
